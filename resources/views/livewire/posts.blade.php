<div>
    <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
        <div class="mt-8 bg-white dark:bg-gray-800 overflow-hidden shadow sm:rounded-lg px-4 py-4">
            ...
            @if ($isOpen)
                @include('livewire.create', compact('rd'))
            @endif
            ...
            <table class="table-fixed w-full text-center">
                <thead class="bg-gradient-to-r from-purple-900 via-purple-400 to-pink-400">
                    <tr>
                        ...
                        <th class="w-40 px-4 py-2 text-white">Team</th>
                        <th class="w-40 px-4 py-2 text-white">Name</th>
                        <th class="w-20 px-4 py-2 text-white">Point</th>
                        <th class="w-20 px-4 py-2 text-white">Ranking</th>
                ...
                <tbody class="text-center">
                    ...
                    <tr>
                        {{-- <td>{{ $post->id }}</td> --}}
                        <td>{{ $post->rider->team }}</td>
                        <td>{{ $post->name }}</td>
                        <td>{{ $post->point }}</td>
                        <td>{{ $post->ranking }}</td>
                        <td class="py-2">
                            <button wire:click="edit({{ $post->id }})" class="w-30 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-2 rounded">
                                edit
                            </button>
                            <button wire:click="delShowModal()" class="w-30 bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-2 rounded">
                                delete
                            </button>
